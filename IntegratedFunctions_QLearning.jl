...
=#
    ...
    function IntegratedFnc(YearFileLocation, OutputFileName)
        ...
        State = []
        StateSpace = 511 #I don't fully understand how the state is being encoded. Need to ask Daniel to explain.
        ActionSpace = 7
        ...
        DataFileName = OutputFileName * "_" * "Data"
        PolicyFileName = OutputFileName * "_" * "Policy"

        open(DataFileName, "w") do io
            write(io, "State",",","StateRanks",",","Action",",","NextState",",","NextStateRanks",",","Reward")
         end;

         open(PolicyFileName, "w") do io
            write(io, "State",",", "Action")
         end;
